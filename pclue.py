...
def partida0(ronda, nom_jugador):
	as_ver = ronda[0]
	arma_real = ronda [1]
	lugar_real = ronda [2]
	...
	resultado_asesino = (psospechoso[as_ju-1] == as_ver )
...
def partida1(ronda, nom_jugador):
	as_ver = ronda[0]
	arma_real = ronda [1]
	lugar_real = ronda [2]
	optds= 0
...
def partida2(ronda, nom_jugador):
	as_ver = ronda[0]
	arma_real = ronda [1]
	lugar_real = ronda [2]
...
def partida3(ronda, nom_jugador):
	as_ver = ronda[0]
	arma_real = ronda [1]
	lugar_real = ronda [2]
...
def partida4(ronda, nom_jugador):
	as_ver = ronda [0]
	arma_real = ronda [1]
	lugar_real = ronda [2]
...
psospechoso = ["Fernando", "Yasser", "Pedro", "Victor", "Jorge "]
armas = ["Machete", "Veneno para ratas","Almohada", "Destornillador", "arma de fuego"]
lugares = ["Jardin","Sala ","Dormitorio","Baño","cocina "]

al= random.randrange(4)

print ("Bienvenido, intenta descubrir que fue lo que sucedio. desde este momento eres el detective: ")

nombre_jugador = input("Escribe tu nombre:")
clearConsole()
print("Detective", nombre_jugador,"Tenemos 5 sospechos, 5 objetos y 5 locaciones, ¿Podras resolver el misterio? Comencemos!!")


if al== 0:
	ronda = [psospechoso[0], armas[2], lugares[0]]
	partida0(ronda,nombre_jugador)
	
if al== 1:
	ronda = [psospechoso[0], armas[1], lugares[1]]
	partida1(ronda,nombre_jugador)
	
if al== 2:
	ronda = [psospechoso[0], armas[2], lugares[2]]
	partida2(ronda,nombre_jugador)
	
if al== 3:
	ronda = [psospechoso[3], armas[3], lugares[3]]
	partida3(ronda,nombre_jugador)
	
if al== 4:
	ronda = [psospechoso[4], armas[4], lugares[2]]
	partida4(ronda,nombre_jugador)
